    ...
    public void widgetClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.earning_commit:
                saveData();
                CommitUtil.commitDelayInfo(context, taskNo, new CommitUtil.CommitCallBack() {
                    @Override
            ...
            switch (requestCode) {
                case 0x13:
                    if (data.getStringExtra("address") != null) {
                        companyAddressEdit.setText(data.getStringExtra("address"));
                        companyAddressEdit.setSelection(data.getStringExtra("address").length());
                    }
                    break;
                case 0x12:
