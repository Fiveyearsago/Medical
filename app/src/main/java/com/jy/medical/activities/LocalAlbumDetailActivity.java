...
import com.jy.medical.util.ExtraKey;
 ...
 * @date 2015-4-11
 */
public class LocalAlbumDetailActivity extends BaseActivity implements MatrixImageView.OnSingleTapListener, View.OnClickListener
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        if (folders != null) {
                            ...
                            MyAdapter adapter = new MyAdapter(LocalAlbumDetailActivity.this, folders);
                            title.setText(folder);
                            gridView.setAdapter(adapter);
                            //设置当前选中数量
                            if (checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize() > 0) {
                                finish.setText("确定(" + (checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize()) + ")");
                                finish.setEnabled(true);
                                headerFinish.setText("确定(" + (checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize()) + ")");
...
//                                headerFinish.setEnabled(false);
    ...
    public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
        if (!b) {
            ...
            if (!checkedItems.contains(compoundButton.getTag())) {
                if(checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize()>=100){
                    Toast.makeText(this,"最多选择100张图片", Toast.LENGTH_SHORT).show();
    ...
    public class MyAdapter extends BaseAdapter {
        ...
        public MyAdapter(Context context, List<LocalImageHelper.LocalFile> paths) {
            ...
            options=new DisplayImageOptions.Builder()
                    ...
                    .setImageSize(new ImageSize(((MedicalApplication) context.getApplicationContext()).getQuarterWidth(), 0))
